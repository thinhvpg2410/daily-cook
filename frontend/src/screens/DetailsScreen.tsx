...
type RecipeDetail = {
    ...
    likes?: number | null;
    steps?: string[];
    items?: { amount: number; unitOverride?: string | null; ingredient: { name: string; unit?: string | null } }[];
};
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={s.section}>
                    ...
                    <View style={s.row}>
                        ...
                        <Text style={{marginLeft: 6, color: "#555"}}>{(detail?.cookTime ?? 30)} min</Text> </View>
                    ...
                    {detail?.items?.length ? (
                        detail.items.map((it, idx) => (
                            <Text key={idx} style={s.ing}>
                                • {it.amount}{it.unitOverride || it.ingredient.unit ? ` ${it.unitOverride || it.ingredient.unit}` : ""} {it.ingredient.name}
                            </Text>
                        ))
                    ) : (
                        [
